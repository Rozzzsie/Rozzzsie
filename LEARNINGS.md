...
## The instrument inside the system: diagnostics that route through the thing under repair

**2026-08-28 (a multi-hour infrastructure debug; n=3 instances in a single session)**

One troubleshooting arc produced the same structural error three times, each costing a round of work. (1) **Recovery channel.** The control surface needed to reconfigure a component was itself reachable only *through* that component — so the repair tool went down with the thing it was meant to repair, and every attempt to pull a fresh configuration failed for reasons that had nothing to do with the configuration. (2) **Test target.** Three consecutive fix attempts were judged by loading a target that shared an address with the component under repair, making a working fix and a broken one indistinguishable at the point of measurement; the flaw was real but, when finally corrected, did not change the verdict — a valid methodological criticism is not itself evidence. (3) **Symptom string.** One error message covered three genuinely different faults — nothing listening, reached-and-rejected, and a client never told the subsystem existed — because the string named the observer's experience rather than any cause, silently merging three problems into one apparent recurrence. The actual fault was in the only component never inspected, because it had never looked broken from outside.

**Rule.** *Before trusting a diagnostic, establish that it does not share fate with the subsystem under repair. Three surfaces carry this dependency: the **recovery channel** (if the thing is down, can I still reach what fixes it?), the **test target** (does my success signal share an address, host, or path with the component?), and the **symptom string** (does this message name a cause, or only what the observer saw?). Where a dependency exists, secure an out-of-band path before diagnosing — a second route to the control surface, a test target with no shared address, and at least one measurement read from the failing component's own logs rather than inferred from outside it.* A shared symptom is not a shared cause: three faults wearing one error message read as one recurring fault until something splits them, and the splitting instrument must come from outside the fault's blast radius.
