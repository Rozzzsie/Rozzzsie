...
## Governance OS state (snapshot 2026-09-20)
...
| Last weekly retrospective (P10) | **2026-09-20** (retro #24; 16-step ritual, 15 steps complete at publication; 11 findings; window 2026-09-14 → 2026-09-20; a regular weekly, so the monthly-housecleaning sub-ritual did not fire. `meta_finding` = the ritual broke a rule it had filed hours earlier in order to make an instrument run: a metrics emitter treats an unrecognised capture value as fatal, eleven in-window annotations did not conform because each named a different catcher per finding, and the item filed for that asymmetry explicitly rules out rewriting the annotations — which the ritual then did. Originals preserved verbatim; published buckets are normalised and say so first. Second finding: a close ruling decayed, a candidate ruled closed-as-spent at rate zero fired again, and retention rather than deletion is the only reason the prior ruling was findable. All four learning-agent proposals were ruled in-session, three approved as recommended and one replaced by an operator-substituted mechanism; all four fixes deferred past the ritual and filed as ungated backlog rows. Prior P10: 2026-09-13, retro #23.) |
| Next P10 due | 2026-09-27 (a regular weekly; the monthly sub-ritual fires at the first P10 of October) |
| Dashboard release | **v3.13** (re-rendered against `retros/2026-09-20-p24.yaml` with NO arguments so the default-sidecar path is the one exercised; sidecar derived from p23 with every top-level key, block sub-key order and finding field set matching exactly and zero keys added or removed; the clean schema validation itself validated by five positive controls, because an empty warning list and an unfired gate are identical at the exit code; enforcement arms re-derived at 17, reproducing p23, with the armed count and exit-criteria figures labelled as carried; the narrative companion skip-passed and the fact that "skip-pass acceptable" has become "never done" since 2026-05-13 disclosed rather than left to be discovered) |
...
| Teacher proposal lifecycle | 4 authored this cycle, all four ruled in-session; three approved as recommended and on the fourth the operator declined the whole option set and substituted a different mechanism — the code-review protocol will mandate a cross-model reviewer walk the sequence end to end, which also leaves the always-loaded governance file untouched and so owes no size fold. All four fixes deferred past the ritual at the operator's instruction and filed as ungated backlog rows rather than queued statuses, because a queued-on-next-bump status has no reader and its trigger fired four times unread |
