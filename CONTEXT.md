...
## Governance OS state (snapshot 2026-09-13)
...
| Last weekly retrospective (P10) | **2026-09-13** (retro #23; 16-step ritual; 11 findings; window 2026-09-06 → 2026-09-13; a regular weekly, so the monthly-housecleaning sub-ritual did not fire. `meta_finding` = the retrospective's own bookkeeping was the instrument that failed, three times in one direction: an edit count published as a stale count, an approved build recorded as filed and on no ledger, and four carried findings on no work list. Each record was true about what it did and silent about what it did not; one census run for another reason caught all three. All three learning-agent proposals were ruled and shipped in-cycle; the operator declined to widen two new rules past their named bounds without a measured instance. Prior P10: 2026-09-06, retro #22.) |
| Next P10 due | 2026-09-20 (a regular weekly; the monthly sub-ritual fires at the first P10 of October) |
| Dashboard release | **v3.12** (re-rendered against `retros/2026-09-13-p23.yaml` with NO arguments so the default-sidecar path is the one exercised; sidecar derived from p22 with key order verified by comparison; enforcement arms continued 15 → 17 only after an overlap test reproduced p22's figure; carried figures labelled as carried; the stale "Current release" header in the dashboard README corrected and the missing v3.11 note disclosed) |
