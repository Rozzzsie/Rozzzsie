...
Rozzzsie runs a governance OS (v3.9.3) with four layers the fam operates under:
...
| 2 | **Versioned governance protocols + symlink-canonical** | Canonical filename carries the version (`agent-protocols-3.9.3.md`); a stable symlink (`agent-protocols.md`) retargets on bumps so references never break. | [`_config/`](_config/) (canonical + symlink) |
...
- [`dashboard/`](dashboard/) — **the OS observability layer**, currently at **v1.3** ([live at `rozzzsie.github.io/Rozzzsie/dashboard/`](https://rozzzsie.github.io/Rozzzsie/dashboard/), tagged `dashboard-v1.3`). Renders the most recent P10 retro from its YAML sidecar into a static HTML dashboard. Frame 1 + Frame 3 stacked per Luma's 2026-04-25 evening consult (subtree-in-Rozzzsie-public for consistency-with-spine, scope-honest single-retro v1); v1.2 layered Luma's second consult (layer-mismatch reframe); v1.3 re-renders against the v3.9.x ship-cycle sidecar with v3.9.3 Pn-token cascade in the renderer + real graceful-suppression for redacted (null) discipline + latency fields. Full release notes in [`dashboard/README.md`](dashboard/README.md).
