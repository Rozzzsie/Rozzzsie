...
**Live dashboard (v2.0 — sprint-2 unlocked):** [**rozzzsie.github.io/Rozzzsie/dashboard/**](https://rozzzsie.github.io/Rozzzsie/dashboard/) — the OS observing itself. Decision velocity, discipline metrics, Luma tally by category, proposal backlog cohort flow, latency observations, full findings detail with status pills, **AND multi-retro governance trend rendering** (new in v2.0: 3 metric cards × 3 P10 cycles — checkpoint miss rate / decision velocity / Teacher invocations). Renders mechanically from the most recent P10 retro sidecar at [`retros/`](retros/) on every push to `main`; trend lines auto-extend as new sidecars accumulate. Sprint-2 v2.0 ships against `retros/2026-05-10-p5.yaml` for the single-retro panels + all 3 sidecars (`p3 + p4 + p5`) for the trend chart. First sidecar with parallel public narrative companion at [`_retro/2026-05-10-p10-retro.md`](_retro/2026-05-10-p10-retro.md). Release notes in [`dashboard/README.md`](dashboard/README.md); tagged in git as `dashboard-v2.0`. The arxiv anchor is [EDD 2024](https://arxiv.org/abs/2411.13768) — *evaluation as continuous governing function, not terminal checkpoint.*
...
| 2 | **Versioned governance protocols + symlink-canonical** | Canonical filename carries the version (`agent-protocols-3.10.5.md`); a stable symlink (`agent-protocols.md`) retargets on bumps so references never break. | [`_config/`](_config/) (canonical + symlink) |
...
- **Dashboard v3.11 — the enforcement note stopped asserting and started deriving.** Its coverage caption had carried `"check-level coverage is zero"` as fixed prose. That was true the day it was written and became false the first cycle a check stated an exit condition; at p22 it would have printed beside a measured 3 of 6. A narrative pinned beside a number it does not read is a published falsehood with no failing test, so the caption now derives from the measurement. Same release registers **and renders** the arm-unit declaration: the enforcement series changed units this cycle (five built / four armed, unreproducible → fifteen validator arms enumerated from the commit-boundary hook), and a changed unit published without saying so is how a series quietly stops being a series.
- **`detection_source_distribution` ships empty by construction, and is the SECOND efficacy metric — not the first.** A block grading who-caught-it over P10 findings has been required and rendered since p18. The new block grades LEARNINGS entries in six buckets rather than three; the populations differ, the vocabularies are unmapped, and both blocks now say so on the face of the card. The `nothing` bucket is the point of the metric: it means the enforcement corpus did not catch the defect, and it is the number a compliance metric can never round up into a success.
- **A house rule fell out of building it: a schema key is DECLARED and RENDERED, or it is not shipped.** Registering a key in the manifest silences the unknown-key warning while publishing nothing — measured on a key registered since v3.10 and read by no code path. Three keys hit this in one session and were folded into rendered fields instead.
...
- [`dashboard/`](dashboard/) — **the OS observability layer**, currently at **v2.0** ([live at `rozzzsie.github.io/Rozzzsie/dashboard/`](https://rozzzsie.github.io/Rozzzsie/dashboard/), tagged `dashboard-v2.0`). Renders the most recent P10 retro from its YAML sidecar into a static HTML dashboard PLUS **multi-retro governance trend rendering** across all accumulated sidecars (sprint-2 unlock). Frame 1 + Frame 3 stacked per Luma's 2026-04-25 evening consult (subtree-in-Rozzzsie-public for consistency-with-spine, scope-honest single-retro v1); v1.2 layered Luma's second consult (layer-mismatch reframe); v1.3 re-renders against the v3.9.x ship-cycle sidecar with v3.9.3 Pn-token cascade + graceful-suppression for redacted (null) discipline + latency fields; v1.4 adds Frame 2 fam-dispatch widget (sub-band split for dispatch + reactions axes) + measurement-surface anchor + Luma reframe-axis tally relocated to deep-dive panel; v1.5 re-renders against the v3.10.x ship-cycle sidecar (`2026-05-10-p5.yaml`) + first sidecar with parallel public narrative companion at `_retro/2026-05-10-p10-retro.md`; **v2.0 unlocks sprint-2** — 3 metric cards (checkpoint miss rate / decision velocity / Teacher invocations) × 3 P10 cycles via inline SVG sparklines + directional annotations + apples-to-apples caveats where measurement-shape isn't yet stable. Full release notes in [`dashboard/README.md`](dashboard/README.md).
