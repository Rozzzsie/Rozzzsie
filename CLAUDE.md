...
- **Canonical file** carries the full version in its filename: `_config/agent-protocols-3.11.md` (current). When version bumps to 3.10.5 or 4.0.0, the canonical is renamed to match.
- **Stable symlink** sits alongside: `_config/agent-protocols.md → agent-protocols-3.11.md`. All live references — workspace CLAUDE.md files, hooks, output-checklist, roles-map, design docs — point at the symlink path (`_config/agent-protocols.md`). The symlink retargets on version bump; references never need updating.
