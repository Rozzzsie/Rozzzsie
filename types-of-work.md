...
→ *Live:* two product-docs sync pipelines (one external customer-facing knowledge surface, one internal Knowledge Bank)
→ *Could extend to:* any team's Confluence, Notion, or internal wiki

**Slack knowledge agent**
A support rep @mentions the bot, it searches the internal playbook, and replies in-thread with grounded, sourced answers — no human researcher in the loop.
→ *Live:* internal playbook agent (210-page playbook + 73 FAQ cases)
...
→ *Live:* ticket triage skill — parse exported ticket → classify (inquiry / problem / admin request) → draft customer reply + handoff summary
...
→ *Live:* AI-bot empowerment council — recurring catchup-note processing
...
→ *Live:* weekly AI-bot regional performance snapshots (deflection rate, CX score, CSAT) + Google Slides auto-deck
...
→ *Live:* hiring scorecards + narratives; team performance summaries; senior-IC operating-pattern artifacts
...
→ *Live:* internal product-support snippet library (37 snippets — each with Technical Notes for human agents + Customer-Facing Response + Bot Action for the AI rep)
